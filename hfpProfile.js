...
class HfpProfile extends EventEmitter {
    ...
    /**
     * @param {object} opts
     * @param {import('@deltachat/dbus-next').MessageBus} opts.bus
    ...
    async _initSLC(mac) {
        try {
            ...
            // Step 5: Enable caller ID presentation
            await this._sendCommand(mac, 'AT+CLIP=1');

            // Step 6: Enable call waiting notifications
            await this._sendCommand(mac, 'AT+CCWA=1');

            // Step 7: Enable extended error codes
            await this._sendCommand(mac, 'AT+CMEE=1');

            // Step 8: Query operator name
