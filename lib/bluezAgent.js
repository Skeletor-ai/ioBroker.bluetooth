...
class BluezAgent {
    ...
    /**
     * @param {object} opts
     * @param {import('@deltachat/dbus-next').MessageBus} opts.bus – system bus
    ...
    constructor(opts) {
        ...
        /** @type {Map<string, { resolve: Function, reject: Function, timer: NodeJS.Timeout, type?: string }>} */
     ...
     * For passkey/pin requests, pass the value to resolve with.
     * @param {string} mac
     * @param {string|number} [value] – passkey (number) or PIN (string) for input requests
     */
    confirmPairing(mac, value) {
        const key = this._normalise(mac);
        const pending = this._pending.get(key);
        if (pending) {
            clearTimeout(pending.timer);
            if (pending.type === 'passkey') {
                pending.resolve(Number(value) || 0);
            } else if (pending.type === 'pin') {
                pending.resolve(String(value || '0000'));
            } else {
                pending.resolve();
            }
            this._pending.delete(key);
            this.log.info(`Pairing confirmed for ${mac} (type: ${pending.type || 'confirmation'})`);
        } else {
    ...
    _exportAgent() {
        ...
        class Agent1 extends DbusInterface {
            ...
            RequestPinCode(device) {
                return self._handleRequestInput(device, 'pin');
            }
            ...
            RequestPasskey(device) {
                return self._handleRequestInput(device, 'passkey');
            }
    ...
    /**
     * Handle RequestPinCode / RequestPasskey – wait for user to enter the code.
     *
     * @param {string} devicePath
     * @param {'pin'|'passkey'} type
     * @returns {Promise<string|number>}
     * @private
     */
    _handleRequestInput(devicePath, type) {
        const mac = this._devicePathToMac(devicePath);
        const key = this._normalise(mac);

        this.log.info(`Request${type === 'pin' ? 'PinCode' : 'Passkey'} for ${mac} – waiting for user input`);

        return new Promise((resolve, reject) => {
            const pending = { resolve, reject, type };

            pending.timer = setTimeout(() => {
                this._pending.delete(key);
                reject(new dbus.DBusError('org.bluez.Error.Rejected', `${type} input timeout`));
                this.log.warn(`${type} input timeout for ${mac}`);
            }, this.timeout);

            this._pending.set(key, pending);

            this._emitPairingRequest(devicePath, type, null);
        });
    }
